...
import com.born.util.Xls_Reader;
...
fs = new FileInputStream("src/test/java/com/born/config/or.properties");
OR = new Properties();
OR.load(fs);
...
test.start();
	...
	public void start() throws IllegalAccessException, IllegalArgumentException, InvocationTargetException,
		...
		suiteXLS = new Xls_Reader("src/test/java/com/born/xls/Suite.xlsx");
		...
		for (currentSuiteID = 2; currentSuiteID <= suiteXLS.getRowCount(Constants.TEST_SUITE_SHEET); currentSuiteID++) {
			...
			if (suiteXLS.getCellData(Constants.TEST_SUITE_SHEET, Constants.RUNMODE, currentSuiteID)
				...
				currentTestSuiteXLS = new Xls_Reader("src/test/java/com/born/xls/" + currentTestSuite + ".xlsx");
